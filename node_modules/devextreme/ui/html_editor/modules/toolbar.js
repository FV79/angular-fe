...
var ToolbarModule = function(_BaseModule) {
    ...
    function ToolbarModule(quill, options) {
        ...
        if ((0, _type.isDefined)(options.items)) {
            ...
            _this.quill.on("editor-change", function(eventName) {
                ...
                _this.updateFormatWidgets(isSelectionChanged);
                _this.updateHistoryWidgets()
            })
        }
        return _this
    }
    _createClass(ToolbarModule, [{
        ...
        value: function(formatName, isApplied, formats) {
            var widget = this._formats[formatName];
            if (!widget) {
                return
            }
            if (isApplied) {
        ...
        value: function() {
            ...
            return function() {
                ...
                var promise = _this5._editorInstance.showFormDialog({
                    formData: formData,
                    items: _this5._getLinkFormItems()
                });
                promise.done(function(formData) {
                    ...
                    if (selection && !formats.link) {
                        ...
                        length && _this5.quill.deleteText(index, length);
                        _this5.quill.insertText(index, text, "link", formData, USER_ACTION);
                        _this5.quill.setSelection(index + text.length, 0)
                    } else {
        ...
        value: function() {
            ...
            return function() {
                ...
                var isUpdateDialog = formData.hasOwnProperty("src");
                var selection = _this6.quill.getSelection();
                var pasteIndex = selection && selection.index || _this6.quill.getLength();
                ...
                promise.done(function(formData) {
                    if (isUpdateDialog) {
                        var formatIndex = selection && !selection.length && selection.index - 1 || pasteIndex;
                        _this6.quill.formatText(formatIndex, 1, {
                            width: formData.width,
                            height: formData.height,
                            alt: formData.alt
                        }, USER_ACTION)
                    } else {
                        _this6.quill.insertEmbed(pasteIndex, "extendedImage", formData, USER_ACTION)
                    }
                })
            }
        }
        ...
        value: function() {
            ...
            var $toolbar = (0, _renderer2.default)("<div>").addClass(TOOLBAR_CLASS).appendTo(container);
            this._$toolbarContainer = (0, _renderer2.default)(container).addClass(TOOLBAR_WRAPPER_CLASS);
            _events_engine2.default.on($toolbar, (0, _utils.addNamespace)("mousedown", this._editorInstance.NAME), function(e) {
                e.preventDefault()
            });
            this.toolbarInstance = this._editorInstance._createComponent($toolbar, _toolbar2.default, this.toolbarConfig);
                ...
                options: {
                    onInitialized: function(e) {
                        if (item.formatName) {
                            ...
                            _this11._formats[item.formatName] = e.component
                        }
                    }
                }
            };
        ...
        value: function(isResetRequired) {
            ...
            for (var formatName in formats) {
                ...
                var formatWidget = this._formats[widgetName] || this._formats[formatName];
                if (!formatWidget) {
                    continue
                }
        ...
        value: function(formatName, color) {
            var formatWidget = this._formats[formatName];
            if (!formatWidget) {
                return
            }
        ...
        value: function() {
            var _this12 = this;
            (0, _iterator.each)(this._formats, function(name, widget) {
