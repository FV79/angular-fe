...
function createTick(axis, renderer, tickOptions, gridOptions, skippedCategory, skipLabels, offset) {
    ...
    return function(value) {
        var tick = {
            ...
            _updateLine: function(lineElement, settings, storedSettings, animate) {
                if (!lineElement) {
                    return
                }
            ...
            drawLabel: function(range) {
                var stubData = axis.getTranslator().getBusinessRange().stubData;
                var labelIsVisible = labelOptions.visible && !skipLabels && !stubData && !axis.areCoordsOutsideAxis(this.labelCoords);
                if (!labelIsVisible) {
                    if (this.label) {
                        this.label.remove()
                    }
                ...
                if (isDefined(text) && "" !== text && !emptyStrRegExp.test(text)) {
                    ...
                    this.updateLabelPosition();
                    var labelHint = axis.formatHint(this.value, labelOptions, range);
            ...
            updateGridPosition: function(animate) {
                this._updateLine(this.grid, axis._getGridPoints(tick.coords), this._storedCoords && axis._getGridPoints(this._storedCoords), animate)
            },
